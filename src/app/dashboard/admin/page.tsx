...
type Shop = { id: string; nombre: string; slug: string | null };
...
export default function AdminPage() {
  ...
  const load = async () => {
    ...
    const { data } = await supabase.from("barberias").select("id, nombre, slug").order("nombre");
    setShops(data || []);
  };
  ...
  return (
    <main className="min-h-screen pb-10" style={{ background: "var(--bg)", color: "var(--text)" }}>
      <div className="max-w-md mx-auto px-5 pt-5">
        ...
        <p className="text-sm mb-6" style={{ color: "var(--muted)" }}>Crear barberías sin mezclar datos</p>
        ...
        {shops.map((s) => (
          <div key={s.id} className="rounded-2xl p-4 mb-3" style={{ background: "var(--card)", border: "1px solid var(--line)" }}>
            <p className="font-medium">{s.nombre}</p>
            <p className="text-sm" style={{ color: "var(--muted)" }}>/b/{s.slug}</p>
            <a href={`/b/${s.slug}`} className="text-sm">Abrir link público</a>
          </div>
        ))}
      </div>
    </main>

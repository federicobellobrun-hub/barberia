...
export default function ClientesPage() {
  ...
  const load = async () => {
    ...
    const [{ data: shop }, { data: c, error: cErr }, { data: t }] = await Promise.all([
      supabase.from("barberias").select("fidelizacion").eq("id", barberiaId).single(),
      supabase.from("clientes").select("id, nombre, telefono, sin_cortesia").eq("barberia_id", barberiaId).order("nombre"),
      supabase.from("turnos").select("cliente_id").eq("barberia_id", barberiaId).eq("estado", "realizado"),
    ]);
  ...
  return (
    <main className="min-h-screen pb-10" style={{ background: "var(--bg)", color: "var(--text)" }}>
      <div className="max-w-md mx-auto px-5 pt-5">
        <BrandHeader left={<Link href={`/dashboard${shopQ}`}>‹</Link>} />
          ...
          .map((c) => {
            ...
            return (
              <div key={c.id} className="rounded-2xl p-4 mb-3" style={{ background: "var(--card)", border: "1px solid var(--line)" }}>
                <Link href={`/dashboard/clientes/${c.id}${shopQ}`}>
                  <div className="flex justify-between">
                    <p className="font-medium">{c.nombre}</p>
                    {toca && (
                      <span className="text-xs px-2 py-1 rounded-full" style={{ background: "#1c1712", color: "#f4efe6" }}>
                        Cortesía
                      </span>
                    )}
                  </div>
                  <p className="text-sm" style={{ color: "var(--muted)" }}>
                    {c.telefono} · {c.cortes} cortes
                  </p>
                </Link>
                {fidelizacion && c.cortes > 0 && c.cortes % 10 === 0 && (
                  <button onClick={() => void toggleCortesia(c)} className="text-sm mt-2">

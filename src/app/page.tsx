  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [f, h] = await Promise.all([
        supabase.from("fotos").select("id, url").eq("mostrar_inicio", true).order("created_at", { ascending: false }).limit(6),
  ...
  return (
    <main className="min-h-screen pb-28" style={{ background: "var(--bg)", color: "var(--text)" }}>
      <div className="max-w-md mx-auto px-5 pt-5">
        ...
        <p className="text-center text-[11px] tracking-[0.2em] uppercase mb-3" style={{ color: "var(--muted)" }}>
          Est. Montevideo
        </p>
        <h1 className="text-[36px] font-semibold tracking-tight leading-9 text-center mb-3">
          Reservá tu turno
        </h1>
        ...
        <Link href="/reservar" className="block text-center rounded-2xl py-4 text-base font-medium mb-3" style={{ background: "#1c1712", color: "#f4efe6" }}>
          Reservar
        </Link>
        <Link href="/tienda" className="block text-center rounded-2xl py-4 text-base mb-3" style={{ background: "var(--card)", border: "1px solid var(--line)" }}>
          Productos
        </Link>

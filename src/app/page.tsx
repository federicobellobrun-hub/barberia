  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [f, h] = await Promise.all([
        supabase.from("fotos").select("id, url").order("created_at", { ascending: false }).limit(6),
        supabase.from("horario_semanal").select("dia_semana, hora_inicio, hora_fin, activo").order("dia_semana"),
      ]);
      setFotos(f.data || []);
      setHorarios(h.data || []);
    };
  ...
  return (
    <main className="min-h-screen pb-24" style={{ background: "var(--bg)", color: "var(--text)" }}>
      <div className="max-w-md mx-auto px-5 pt-4">
        ...
        <p className="mt-2 mb-8" style={{ color: "var(--muted)" }}>Agenda simple. Atención precisa.</p>

        <Link href="/reservar" className="block text-center rounded-2xl py-4 font-medium mb-3" style={{ background: "#1d1d1f", color: "#fff" }}>
          Reservar
        </Link>
        <Link href="/tienda" className="block text-center rounded-2xl py-4 mb-3" style={{ background: "var(--card)", border: "1px solid var(--line)" }}>
          Productos
        </Link>
        <Link href="/login" className="block text-center rounded-2xl py-4 mb-8" style={{ background: "var(--card)", border: "1px solid var(--line)" }}>
          Panel del barbero
        </Link>
        ...
        {horarios.length > 0 && (
          <section className="mb-8">
            ...
            <div className="rounded-2xl overflow-hidden" style={{ background: "var(--card)", border: "1px solid var(--line)" }}>
              {horarios.map((h) => (
                <div key={h.dia_semana} className="px-4 py-3 flex justify-between text-sm border-b" style={{ borderColor: "var(--line)" }}>

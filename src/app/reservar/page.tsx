...
export default function ReservarPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: s } = await supabase
        .from("barberias")
        .select("id, nombre, mercado_pago_url, datos_cuenta")
        .eq("slug", slug)
        .maybeSingle();
      if (!s) return;
      setShop(s as Shop);
  ...
  const reservar = async () => {
    ...
    try {
      ...
      setOk(pideSenia ? "Pedido enviado. Queda pendiente hasta confirmar la seña." : "Reserva confirmada");
    } catch (e) {
    ...
    return (
      <main className="mx-auto max-w-md px-4 py-10 text-center">
        ...
        {pideSenia && pago === "mp" && shop?.mercado_pago_url && (
          <a
            href={shop.mercado_pago_url}
            target="_blank"
            rel="noreferrer"
            className="mt-5 inline-block px-6 py-3"
            style={{ background: "var(--text)", color: "var(--bg)", borderRadius: 999 }}
          >
            Ir a pagar la seña
          </a>
        )}
    ...
    <main className="mx-auto min-h-screen max-w-md px-4 pb-24 pt-4">
          ...
          {categorias.map(([c, info]) => (
            ...
            >
              <span className="h-14 w-14 shrink-0 overflow-hidden" style={{ borderRadius: 10, background: "var(--line)" }}>
                {info.foto ? (
                  <img src={info.foto} alt="" className="h-full w-full object-cover transition duration-200 group-hover:scale-110" />
                ) : (
      ...
      {servicio && (
        <div>
          ...
          {pideSenia && (
            <p className="mt-2 text-sm">Seña ${servicio.senia}. El turno queda pendiente hasta que el local confirme el pago.</p>
          )}
